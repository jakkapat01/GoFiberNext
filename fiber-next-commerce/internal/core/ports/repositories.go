...
type UserRepository interface {
	CreateUser(user *domain.User) error                         // สร้างผู้ใช้ใหม่
	FindByID(id uuid.UUID) (*domain.User, error)                // ค้นหาผู้ใช้ตาม ID
	FindByEmail(email string) (*domain.User, error)             // ค้นหาผู้ใช้ตามอีเมล
	Update(user *domain.User) error                             // อัปเดตข้อมูลผู้ใช้
	Delete(id uuid.UUID) error                                  // ลบผู้ใช้ตาม ID
	UpdateRefreshToken(id uuid.UUID, refreshToken string) error // อัปเดต Refresh Token ของผู้ใช้
	List(limit, offset int) ([]domain.User, error)              // รายการผู้ใช้
}
